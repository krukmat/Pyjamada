...
FLOW_PATH="$REPO_ROOT/maestro/systemic-screenshots.yaml"
SCREENSHOTS_DIR="$REPO_ROOT/artifacts/android-systemic-screenshots"
MAESTRO_REPORT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/pyjamada-systemic-maestro.XXXXXX")"
EXPECTED_SCREENSHOTS=(
  "01_systemic_menu.png"
  "02_systemic_run_start.png"
  "03_systemic_bed_wake.png"
  "04_systemic_slippers.png"
  "05_systemic_alarm.png"
  "06_systemic_startled.png"
  "07_systemic_wardrobe.png"
  "08_systemic_success.png"
  "09_systemic_restart.png"
)
...
echo "Capturing systemic Android screens with Maestro..."
...
for screenshot in "${EXPECTED_SCREENSHOTS[@]}"; do
  if [[ ! -s "$SCREENSHOTS_DIR/$screenshot" ]]; then
    echo "Missing systemic screenshot: $screenshot" >&2
    exit 1
  fi
done
...
if [[ "$PNG_COUNT" -ne "$EXPECTED_COUNT" ]]; then
  echo "Expected exactly $EXPECTED_COUNT systemic screenshots, found $PNG_COUNT." >&2
  find "$SCREENSHOTS_DIR" -maxdepth 1 -type f -name '*.png' -printf '%f\n' 2>/dev/null || ls -1 "$SCREENSHOTS_DIR"
  exit 1
fi

echo "$PNG_COUNT systemic Android screenshots available in $SCREENSHOTS_DIR"
