import { DEFAULT_GAME_SETTINGS, applyGameSettingsPatch } from '../src/settings/core/GameSettings';
import { decodeGameSettings, encodeGameSettings, safeDecodeGameSettings } from '../src/settings/core/GameSettingsCodec';
...
async function run() {
  await test('settings use stable defaults when nothing is stored', async () => {
    deepEqual(safeDecodeGameSettings(null), DEFAULT_GAME_SETTINGS, 'default settings');
  });

  await test('settings persist audio, volumes and touch layout', async () => {
  ...
  await test('volume settings clamp to 0..1 increments', async () => {
  ...
  await test('settings codec round-trips valid settings', async () => {
  ...
  await test('malformed or incompatible settings fall back safely', async () => {
