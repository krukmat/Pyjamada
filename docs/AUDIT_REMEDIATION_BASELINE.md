...
| Local model adapter/model (Low bundle) | ready at baseline | At this baseline snapshot, `ollama` was present at `/usr/local/bin/ollama` and `ollama list` contained the three then-pinned models: `devstral-small-2:24b-instruct-2512-q4_K_M`, `gemma4:26b-a4b-it-qat`, and `gpt-oss:20b`. The bundle was not used under that task's explicit authorization. This is historical readiness evidence; current role bindings are governed by `docs/workflow/AGENT_WORKFLOW_GUIDE.md`. |
